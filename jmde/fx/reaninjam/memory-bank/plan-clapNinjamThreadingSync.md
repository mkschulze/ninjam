...
| **Run thread** | Calls `NJClient::Run()`, handles network | Holds `state_mutex` during `Run()` |
| **UI thread** | Renders ImGui, handles user input | Takes `state_mutex` for NJClient API calls |

### Invariants

1. `state_mutex` protects all NJClient API calls **except** `AudioProc()`.
2. Only the MVP UI-touched config fields (master/metronome) and `cached_status` are `std::atomic`; all other NJClient fields (including other `config_*`) must be accessed under `state_mutex`.
3. Callbacks from `Run()` must **never** take `state_mutex` or touch UI directly.
...
Non-MVP `config_*` fields remain non-atomic and must only be read/written under `state_mutex`.
...
struct NinjamPlugin {
    ...
    // Threading
    std::mutex state_mutex;           // Protects NJClient API (except AudioProc)
...
Callbacks invoked from `NJClient::Run()` execute on the Run thread while `state_mutex` is held. They must:
- **NOT** acquire `state_mutex` (deadlock)
...
**Known behavior**: While the license dialog is open, the Run thread is blocked waiting on `license_cv`. Any UI action requiring `state_mutex` (Connect, Disconnect, channel changes) will block until the user responds.
...
void render_remote_channels(NinjamPlugin* plugin) {
    // Remote channel state requires state_mutex
    std::lock_guard<std::mutex> lock(plugin->state_mutex);
...
| Concern | Solution |
|---------|----------|
| Data race on `config_*` | Atomic fields in modified NJClient copy (MVP UI-touched only; other `config_*` under `state_mutex`) |
...
| Callback deadlocks | Callbacks only set flags/enqueue; never lock `state_mutex` |
